  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!ReponseSoumissionFlux.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "The required field(s) %s in ReponseSoumissionFlux is not found in the empty JSON string", ReponseSoumissionFlux.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!ReponseSoumissionFlux.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "The field `%s` in the JSON string is not defined in the `ReponseSoumissionFlux` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if (!jsonObj.get("syntaxe_flux").isJsonPrimitive()) {
        throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "Expected the field `syntaxe_flux` to be a primitive type in the JSON string but got `%s`", jsonObj.get("syntaxe_flux").toString()));
      }
      if ((jsonObj.get("profil_flux") != null && !jsonObj.get("profil_flux").isJsonNull()) && !jsonObj.get("profil_flux").isJsonPrimitive()) {
        throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "Expected the field `profil_flux` to be a primitive type in the JSON string but got `%s`", jsonObj.get("profil_flux").toString()));
      }
