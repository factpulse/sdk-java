...
public class Quantite extends AbstractOpenApiSchema {
    private static final Logger log = Logger.getLogger(Quantite.class.getName());

    public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
        @SuppressWarnings("unchecked")
        @Override
        public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
            if (!Quantite.class.isAssignableFrom(type.getRawType())) {
                return null; // this class only serializes 'Quantite' and its subtypes
            }
            ...
            return (TypeAdapter<T>) new TypeAdapter<Quantite>() {
                @Override
                public void write(JsonWriter out, Quantite value) throws IOException {
                ...
                @Override
                public Quantite read(JsonReader in) throws IOException {
                    ...
                    throw new IOException(String.format(Locale.ROOT, "Failed deserialization for Quantite: no class matches result, expected at least 1. Detailed failure message for anyOf schemas: %s. JSON: %s", errorMessages, jsonElement.toString()));
                }
            }.nullSafe();
        }
    }
     ...
     * @throws IOException if the JSON Element is invalid with respect to Quantite
     */
    public static void validateJsonElement(JsonElement jsonElement) throws IOException {
        ...
        throw new IOException(String.format(Locale.ROOT, "The JSON string is invalid for Quantite with anyOf schemas: BigDecimal, String. no class match the result, expected at least 1. Detailed failure message for anyOf schemas: %s. JSON: %s", errorMessages, jsonElement.toString()));
    }

    /**
     * Create an instance of Quantite given an JSON string
     *
     * @param jsonString JSON string
     * @return An instance of Quantite
     * @throws IOException if the JSON string is invalid with respect to Quantite
     */
    public static Quantite fromJson(String jsonString) throws IOException {
        return JSON.getGson().fromJson(jsonString, Quantite.class);
    }

    /**
     * Convert an instance of Quantite to an JSON string
     *
     * @return JSON string
     */
