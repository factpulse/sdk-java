  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!MontantTotal.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "The required field(s) %s in MontantTotal is not found in the empty JSON string", MontantTotal.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!MontantTotal.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "The field `%s` in the JSON string is not defined in the `MontantTotal` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if ((jsonObj.get("motifRemiseGlobaleTtc") != null && !jsonObj.get("motifRemiseGlobaleTtc").isJsonNull()) && !jsonObj.get("motifRemiseGlobaleTtc").isJsonPrimitive()) {
        throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "Expected the field `motifRemiseGlobaleTtc` to be a primitive type in the JSON string but got `%s`", jsonObj.get("motifRemiseGlobaleTtc").toString()));
      }
  }
