...
public class ChorusProCredentials {
   ...
   * @throws IOException if the JSON Element is invalid with respect to ChorusProCredentials
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!ChorusProCredentials.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The required field(s) %s in ChorusProCredentials is not found in the empty JSON string", ChorusProCredentials.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!ChorusProCredentials.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The field `%s` in the JSON string is not defined in the `ChorusProCredentials` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if (!jsonObj.get("chorus_pro_login").isJsonPrimitive()) {
        throw new IllegalArgumentException(String.format(Locale.ROOT, "Expected the field `chorus_pro_login` to be a primitive type in the JSON string but got `%s`", jsonObj.get("chorus_pro_login").toString()));
      }
      if (!jsonObj.get("chorus_pro_password").isJsonPrimitive()) {
        throw new IllegalArgumentException(String.format(Locale.ROOT, "Expected the field `chorus_pro_password` to be a primitive type in the JSON string but got `%s`", jsonObj.get("chorus_pro_password").toString()));
      }
  }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!ChorusProCredentials.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'ChorusProCredentials' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<ChorusProCredentials> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(ChorusProCredentials.class));

       return (TypeAdapter<T>) new TypeAdapter<ChorusProCredentials>() {
           @Override
           public void write(JsonWriter out, ChorusProCredentials value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public ChorusProCredentials read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of ChorusProCredentials given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of ChorusProCredentials
   * @throws IOException if the JSON string is invalid with respect to ChorusProCredentials
   */
  public static ChorusProCredentials fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, ChorusProCredentials.class);
  }

  /**
   * Convert an instance of ChorusProCredentials to an JSON string
   *
   * @return JSON string
   */
